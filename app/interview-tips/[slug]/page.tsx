...
export async function generateMetadata(
  ...
  const url = `${SITE_URL}/interview-tips/${slug}`;
...
export default async function InterviewTipsArticle(
  { params }: { params: Promise<{ slug: string }> }
) {
  const { slug } = await params;
  const article = getArticle(slug);
  if (!article || article.section !== "interview-tips") return notFound();

  const color   = tagColors[article.subcategory];
  const related = getArticlesBySection("interview-tips")
      ...
      <div style={{ background: "#f9fafb", borderBottom: "1px solid #e5e7eb", padding: "32px 20px" }}>
        <div className="container" style={{ maxWidth: 760 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16, fontSize: 13, color: "#6b7280", flexWrap: "wrap" }}>
            <Link href="/interview-tips" style={{ color: "#1a56db" }}>Interview Tips</Link>
            <span>→</span>
      ...
      <div className="container" style={{ maxWidth: 760, padding: "48px 20px" }}>
        ...
        {related.length > 0 && (
          <div style={{ marginTop: 56, paddingTop: 32, borderTop: "1px solid #e5e7eb" }}>
            <h3 style={{ fontSize: 18, fontWeight: 600, marginBottom: 20 }}>Related guides</h3>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 14 }}>
              {related.map(r => (
                <Link key={r.slug} href={`/interview-tips/${r.slug}`}
        ...
        <div style={{ marginTop: 32, textAlign: "center" }}>
          <Link href="/interview-tips" style={{ color: "#1a56db", fontSize: 14, fontWeight: 500 }}>← Back to Interview Tips</Link>
        </div>
      </div>
