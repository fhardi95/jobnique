...
import { getArticlesBySection, getFeaturedArticles, interviewTipsSubcategories, tagColors } from "@/lib/articles";

export default function InterviewTipsClient() {
  const [active, setActive] = useState("All");
  const all = getArticlesBySection("interview-tips");
  const featured = getFeaturedArticles("interview-tips");
    ...
    <>
      ...
      <div style={{ background: "#f9fafb", borderBottom: "1px solid #e5e7eb", padding: "48px 20px 40px" }}>
        <div className="container">
          <h1 style={{ fontFamily: "'DM Serif Display',serif", fontSize: "clamp(32px,5vw,48px)", marginBottom: 12 }}>Interview Tips</h1>
          <p style={{ color: "#6b7280", fontSize: 16, maxWidth: 520 }}>
            Expert guides to help you prepare, answer tough questions, handle nerves, and turn every interview into a job offer.
          </p>
        </div>
      </div>
      ...
      <div style={{ borderBottom: "1px solid #e5e7eb", background: "#fff" }}>
        <div className="container" style={{ display: "flex", gap: 8, flexWrap: "wrap", padding: "14px 20px" }}>
          {["All", ...interviewTipsSubcategories].map(cat => (
      ...
      <div style={{ background: "#fff", minHeight: "60vh" }}>
        <div className="container" style={{ padding: "48px 20px" }}>
            ...
            <div style={{ marginBottom: 56 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: 20 }}>
                {featured.map(a => {
                  ...
                  return (
                    <Link key={a.slug} href={`/interview-tips/${a.slug}`}
                      style={{ border: "1px solid #e5e7eb", borderRadius: 12, padding: "24px", display: "block", color: "inherit", textDecoration: "none" }}
          ...
          {/* All articles */}
          <div>
            <h2 style={{ fontSize: 20, fontWeight: 600, marginBottom: 24 }}>{active === "All" ? "All guides" : active}</h2>
