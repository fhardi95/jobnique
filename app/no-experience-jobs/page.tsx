// FILE: app/no-experience-jobs/page.tsx
// Deploy at: jobnique.com/no-experience-jobs
...
export const metadata: Metadata = {
  title: "No Experience Jobs USA 2026 – Entry Level Hiring Now | Jobnique",
  description:
    "Find no experience jobs near you in 2026. Browse entry-level roles across 100+ US cities and all 50 states. No prior experience required. Updated daily.",
  alternates: { canonical: `${BASE_URL}/no-experience-jobs` },
  openGraph: {
    title: "No Experience Jobs USA 2026 – Entry Level Hiring Now",
    description: "Find entry-level jobs with no experience required across 100+ US cities and all 50 states. Updated daily.",
    url: `${BASE_URL}/no-experience-jobs`,
    siteName: "Jobnique",
    type: "website",
  },
    ...
    <>
      ...
      <div style={{ background: "#f9fafb", borderBottom: "1px solid #e5e7eb", padding: "10px 20px" }}>
        <nav style={{ maxWidth: 1200, margin: "0 auto", fontSize: 13, color: "#6b7280" }}>
          ...
          <span style={{ color: "#111827", fontWeight: 500 }}>No Experience Jobs</span>
        </nav>
      </div>
      ...
      <div style={{ background: "linear-gradient(135deg, #1a56db 0%, #1e40af 100%)", padding: "52px 20px 60px", textAlign: "center" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          ...
          <p style={{ fontSize: 17, color: "rgba(255,255,255,0.85)", marginBottom: 28 }}>
            Entry-level jobs across the US — no experience required. Browse by city or state and start applying today.
          </p>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "48px 20px 80px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 14 }}>
            {featured.map((city) => (
              <Link key={city.slug} href={`/no-experience-jobs/${city.slug}`} className="city-card">
                ...
                <p style={{ fontSize: 13, color: "#6b7280" }}>No experience jobs →</p>
              </Link>
            ))}
          </div>
        </section>
        ...
        <section style={{ marginBottom: 52 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: 12 }}>
            {US_STATES.map((state) => (
              <Link key={state.slug} href={`/no-experience-jobs/states/${state.slug}`} className="state-card">
        ...
        <section id="all-cities" style={{ marginBottom: 52 }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {allCities.map((city) => (
              <Link key={city.slug} href={`/no-experience-jobs/${city.slug}`} className="city-pill">
                {city.label}
              </Link>
            ))}
          </div>
        ...
        <section style={{ maxWidth: 760, marginBottom: 48, borderTop: "1px solid #e5e7eb", paddingTop: 48 }}>
          <h2 style={{ fontSize: 22, fontWeight: 700, color: "#0f172a", marginBottom: 16 }}>What Are No Experience Jobs?</h2>
          <p style={{ fontSize: 15, color: "#374151", lineHeight: 1.75, marginBottom: 12 }}>
            No experience jobs — also called entry-level jobs — are positions that don&apos;t require any prior professional work history. They&apos;re ideal for students, recent graduates, career changers, or anyone entering the workforce for the first time. Most employers offer paid on-the-job training so you can learn the role from scratch.
          </p>
          <p style={{ fontSize: 15, color: "#374151", lineHeight: 1.75, marginBottom: 12 }}>
            Common no-experience roles include retail associate, customer service representative, warehouse picker, food service worker, home health aide, delivery driver, and administrative assistant. These positions exist in virtually every US city and state and are posted and filled daily.
          </p>
          <p style={{ fontSize: 15, color: "#374151", lineHeight: 1.75 }}>
            ...
            <Link href="/career-advice/how-to-write-cv-with-no-experience-2026-guide" style={{ color: "#1a56db" }}>CV writing guide</Link>{" "}
