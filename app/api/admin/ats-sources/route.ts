/**
 * app/api/admin/ats-sources/route.ts
 * Security: requireAdminAuth guard added to all handlers.
 * (Business logic is unchanged — only the auth guard was missing.)
 */
...
function parseAtsUrl(url: string): AtsSource | null {
  try {
    ...
    if (u.hostname.includes("greenhouse.io")) {
      ...
      return { platform: "greenhouse", token, company_name: capitalise(token) };
    }

    if (u.hostname === "jobs.lever.co") {
      const token = u.pathname.split("/").filter(Boolean)[0];
      if (!token) return null;
      return { platform: "lever", token, company_name: capitalise(token) };
    }
    ...
    if (u.hostname.includes("myworkdayjobs.com")) {
      ...
      return { platform: "workday", token: `${tenant}:${site}`, company_name: capitalise(tenant) };
    }
